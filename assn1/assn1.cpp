...
const int noSamplesPerFrame = 150, thrsldMultiplier = 5;
...
void setup_global(char* filename){
    FILE *fp;
...
double* count_avg_ambience(char* inputFileName){
    FILE *fp, *op;
    ...
    while(!feof(fp)){
        ...
        if(!isalpha(line[0])){
            x = atof(line);
            if(n == noSamplesPerFrame){
                total_frame++;
                avg[0] += energy/noSamplesPerFrame;
                avg[1] += zcr;
...
void process_word(long int word_s, long int word_e, double energy[], double zcr[]){
...
void word_seggregation(double* ambAvg, char* inputFileName){
    ...
    */
    ...
    for( ; i<lastIndex-4; ++i){
		if(word_start == -1 && word_end == -1 && energy[i+1] > ambAvg[0] * thrsldMultiplier && energy[i+2] > ambAvg[0] * thrsldMultiplier && energy[i+3] > ambAvg[0] * thrsldMultiplier && energy[i+4] > ambAvg[0] * thrsldMultiplier){//
			//marking word start
            word_start = i;
		}
		else if(word_start != -1 && word_end == -1 && energy[i+1] <= ambAvg[0] * thrsldMultiplier && energy[i+2] <= ambAvg[0] * thrsldMultiplier && energy[i+3] <= ambAvg[0] * thrsldMultiplier && energy[i+4] <= ambAvg[0] * thrsldMultiplier){
            //marking word end
            word_end = i;
			//printf("start : %ld, end: %ld, energy at start: %lf & end: %lf\nword_start: %ld, word_end: %ld", word_start*noSamplesPerFrame, word_end*noSamplesPerFrame, energy[word_start], energy[word_end], word_start, word_end);
